...
import torch
import models.bricks as bricks
...
class Attention_center(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.fc = nn.Sequential(
            nn.Linear(in_channels, in_channels, bias=False),  # 原始为256, 后面改成了int(in_channels/2),绿地提取模型为128
            nn.ReLU(inplace=True),
            nn.Linear(in_channels, out_channels, bias=False), # 原始为256, 后面改成了int(in_channels/2),绿地提取模型为128
            nn.Sigmoid()
        )
...
class LightHamHead(nn.Module):
    ...
    def __init__(
        ...
        self.hamburger = Attention_center(in_channels = hidden_channels, out_channels = hidden_channels)
...
class SegNeXt(nn.Module):
    """
    SegNext-Tiny: embed_dims=[3, 32, 64, 160, 256], depths=[3, 3, 5, 2], hidden_channels=256
    SegNext-Small: embed_dims=[3, 64, 128, 320, 512], depths=[2, 2, 4, 2], hidden_channels=256
    SegNext-Big: embed_dims=[3, 64, 128, 320, 512], depths=[3, 3, 12, 3], hidden_channels=512
    SegNext-Large: embed_dims=[3, 64, 128, 320, 512], depths=[3, 5, 27, 3], hidden_channels=1024
    """
    ...
    def __init__(self,
        ...
        if modelType == "Tiny":
            self.encoder = MSCAN(
                embed_dims=[inputChannel, 32, 64, 160, 256],
                expand_ratios=[8, 8, 4, 4],
                depths=[3, 3, 5, 2],
                drop_prob=0.1,
                drop_path_prob=0.1
            ...
            self.encoder = MSCAN(
                embed_dims=[inputChannel, 64, 128, 320, 512],
                expand_ratios=[8, 8, 4, 4],
                depths=[2, 2, 4, 2],
                drop_prob=0.1,
                drop_path_prob=0.1
        ...
        if modelType == "Tiny":
            self.decoder = LightHamHead(
                ...
                hidden_channels=256,
                out_channels=256,
                classes_num=numClass,
                drop_prob=0.1
            )

        elif modelType == "Small":
            self.decoder = LightHamHead(
                in_channels_list=[128, 320, 512],
                hidden_channels=256,
                out_channels=256,
                classes_num=numClass,
                drop_prob=0.1
...
if __name__ == '__main__':
    ...
    net = SegNeXt(inputChannel=2, numClass=2, modelType="Big").to(device)  # 指定输入影像波段数目
