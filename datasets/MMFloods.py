...
#以训练数据为例，写下面的代码逻辑
class MMFloods(BaseDataset):
    def __init__(self,
                 root,#/tmp/pycharm_project_666/data/MMFloods/all_train_data/
                 type,#train
                 list_path, #/tmp/pycharm_project_666/data/list/MMFloods/train_split.lst
                 flip,
                 crop_size,
                 num_classes=2,
                 ignore_label=-1,
        ...
        super(MMFloods, self).__init__(ignore_label, base_size,
                                           mean, std,)
    ...
    def input_transform(self, image):
        ...
        image = image / 255.0
        # 使用指定的均值和标准差进行归一化
        mean = np.array([0.6851, 0.5235])[:, None, None]
        std = np.array([0.0820, 0.1102])[:, None, None]
    ...
    def __getitem__(self, index):
        ...
        if self.type=="test":#测试的数据集
            ...
            label_path="/root/autodl-tmp/pycharm_project_666/data/MMFloods/all_train_label/"+item["label"]
